...
{
    public class CourseReposity(LearnDbContext db): ICourseRepository
    {
        public async Task<List<CourseModel>> GetAll(int? categoryId = null)
        {
            var query = db.Courses.Include(c=>c.Category).AsQueryable();
            if (categoryId == null) query = query.Where(c => c.CategoryId == categoryId.Value);
        ...
        public async Task<CourseModel?> GetDetail(int courseId) => await db.Courses
